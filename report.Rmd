...
```
Classe is kept as a factor since it's the variable we're predicting. All other variables are numeric and can be used for prediction.
...
#Submission
I'll use the provided function to write the appropriate submission files.
```{r}
...
Finally, I'll apply the model to the original test set and output the results for submission.
```{r}
predictionRFimp <- predict(fitRFimp, testing[,-53])
pml_write_files(predictionRFimp)
```